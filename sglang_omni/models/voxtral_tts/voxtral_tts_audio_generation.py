...
def _interleave_qk_weight(
    w: torch.Tensor, n_heads: int, head_dim: int
) -> torch.Tensor:
...
class VoxtralTTSAudioGeneration(nn.Module):
    ...
    def load_weights(self, checkpoint_dir: str) -> None:
            ...
            with safe_open(path, framework="pt", device="cpu") as fp:
                for ckpt_key in fp.keys():
                    ...
                    self._dispatch_weight(
                        ckpt_key, tensor, llm_state, counters,
        ...
        logger.info(
            ...
            counters["llm"], len(missing), len(unexpected),
        )
        if missing:
        ...
        logger.info(
            ...
            counters["acoustic"], counters["embedding"],
        )
